...
class EnhancedTechnicalAnalyzer:
    ...
    def __init__(self):
        ...
        self.lookback_periods = {"short": 5, "medium": 20, "long": 50}

    def analyze_comprehensive_context(
        self, data: pd.DataFrame, current_date: str, lookback_days: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if hasattr(working_data.index, "date") or str(
                working_data.index.dtype
            ).startswith("datetime"):
            ...
            if "date" in working_data.columns:
                working_data["date"] = pd.to_datetime(working_data["date"]).dt.strftime(
                    "%Y-%m-%d"
                )
                ...
                logger.warning(
                    f"No date column found, using last data point as current"
                )
            ...
            context = {
                ...
                "momentum_indicators": self._analyze_momentum(
                    analysis_data, current_data
                ),
                ...
                "bollinger_analysis": self._analyze_bollinger_bands(
                    analysis_data, current_data
                ),
    ...
    def _analyze_price_action(
        self, data: pd.DataFrame, current: pd.Series
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            gap_pct = (
                (gap / data["close"].iloc[-2]) * 100
                if len(data) >= 2 and data["close"].iloc[-2] != 0
                else 0
            )
            ...
            return {
                ...
                "upper_shadow_ratio": round(upper_shadow / current_range, 3)
                if current_range > 0
                else 0,
                "lower_shadow_ratio": round(lower_shadow / current_range, 3)
                if current_range > 0
                else 0,
                "gap": round(gap, 2),
                "gap_pct": round(gap_pct, 2),
                "range": round(current_range, 2),
                "volume_to_avg_ratio": round(
                    current["volume"] / data["volume"].mean(), 2
                )
    ...
    def _analyze_moving_averages(
        ...
        try:
            ...
            ma_5_slope = (
                self._calculate_slope(data["close"].rolling(5).mean(), 3)
                if len(data) >= 8
                else 0
            )
            ma_10_slope = (
                self._calculate_slope(data["close"].rolling(10).mean(), 3)
                if len(data) >= 13
                else 0
            )
            ma_20_slope = (
                self._calculate_slope(data["close"].rolling(20).mean(), 3)
                if len(data) >= 23
                else 0
            )
            ...
            return {
                ...
                "ma_10": round(ma_10, 2),
                "ma_20": round(ma_20, 2),
                "price_vs_ma_5": round(((current_price / ma_5) - 1) * 100, 2)
                if ma_5 != 0
                else 0,
                "price_vs_ma_10": round(((current_price / ma_10) - 1) * 100, 2)
                if ma_10 != 0
                else 0,
                "price_vs_ma_20": round(((current_price / ma_20) - 1) * 100, 2)
                if ma_20 != 0
                else 0,
                ...
                if (
                    current_price > ma_5
                    and current_price > ma_10
                    and current_price > ma_20
                )
                else "no",
                "below_all_mas": "yes"
                if (
    ...
    def _analyze_volume_patterns(
        ...
        try:
            ...
            volume_z_score = (
                (current_volume - avg_volume) / volume_std if volume_std > 0 else 0
            )

            # Volume trend
            volume_trend = (
                self._calculate_slope(data["volume"], 3) if len(data) >= 4 else 0
            )

            # Price-volume relationship
            price_change = (
                current["close"] - data["close"].iloc[-2] if len(data) >= 2 else 0
            )
            ...
            return {
                ...
                if (
                    (price_change > 0 and volume_ratio > 1)
                    or (price_change < 0 and volume_ratio > 1)
                )
                else "no",
            }
        except Exception as e:
    ...
    def _analyze_volatility(
        ...
        try:
            ...
            historical_vol = (
                ...
                if len(data) >= 20
                else pd.Series([volatility])
            )
            vol_percentile = (
                (volatility > historical_vol).mean() * 100
            ...
            return {
                ...
                "atr_vs_price": round((atr / current["close"]) * 100, 2)
                if current["close"] != 0
                else 0,
            }
        except Exception as e:
    ...
    def _analyze_momentum(
        ...
        try:
            ...
            # Rate of Change
            roc_5 = (
                ((current["close"] / data["close"].iloc[-6]) - 1) * 100
                if len(data) >= 6
                else 0
            )
            roc_10 = (
                ((current["close"] / data["close"].iloc[-11]) - 1) * 100
                if len(data) >= 11
                else 0
            )
    ...
    def _analyze_support_resistance(
        ...
        try:
            ...
            resistance_distance = min(
                [(h - current_price) for h in recent_highs if h > current_price],
                default=0,
            )
            support_distance = min(
                [(current_price - l) for l in recent_lows if l < current_price],
                default=0,
            )
            ...
            return {
                "nearest_resistance": round(
                    min(
                        [h for h in recent_highs if h > current_price],
                ...
                "nearest_support": round(
                    max(
                        [l for l in recent_lows if l < current_price],
                        default=current_price,
                    ),
                    2,
                ),
                "resistance_distance_pct": round(
                    (resistance_distance / current_price) * 100, 2
                )
                if current_price != 0
                else 0,
                "support_distance_pct": round(
                    (support_distance / current_price) * 100, 2
                )
                if current_price != 0
                else 0,
    ...
    def _analyze_trend_strength(
        ...
        try:
            ...
            # Classify trend
            trend_direction = (
                "bullish" if slope > 0 else "bearish" if slope < 0 else "neutral"
            )
            ...
            return {
                ...
                "strong_trend": "yes"
                if (trend_strength > 0.7 and adx_value > 25)
                else "no",
                "weak_trend": "yes"
                if (trend_strength < 0.3 or adx_value < 20)
                else "no",
            }
        except Exception as e:
    ...
    def _analyze_bollinger_bands(
        ...
        try:
            ...
            bb_width = (
                ((bb_upper - bb_lower) / bb_middle) * 100 if bb_middle != 0 else 0
            )

            # Band squeeze detection
            bb_width_avg = (
                data["close"].rolling(bb_period).std().rolling(10).mean().iloc[-1]
                if len(data) >= 30
                else bb_std
            )
            ...
            return {
                ...
                if (is_squeeze and bb_position > 0.8 or bb_position < 0.2)
                else "no",
            }
        except Exception as e:
    ...
    def _analyze_macd(self, data: pd.DataFrame, current: pd.Series) -> Dict[str, Any]:
        ...
        try:
            ...
            histogram_trend = (
                self._calculate_slope(histogram, 3) if len(histogram) >= 4 else 0
            )
            ...
            return {
                ...
                "divergence_potential": "yes"
                if (abs(histogram_trend) > 0.001)
                else "no",
    ...
    def _empty_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "volume_analysis": {"volume_trend": "normal", "volume_strength": "average"},
            "volatility_analysis": {
            ...
            "market_regime": {"regime": "consolidation", "confidence": 0.5},
            "bollinger_analysis": {
                "position": "middle",
                "squeeze": "no",
                "expansion": "no",
    ...
    def _detect_price_volume_divergence(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            return (price_trend > 0 and volume_trend < 0) or (
    ...
    def _calculate_rsi(self, close_prices: pd.Series, period: int = 14) -> float:
        ...
        try:
            ...
            gain = (
                ...
                .rolling(window=min(period, len(delta)))
                .mean()
            )
            loss = (
                (-delta.where(delta < 0, 0))
    ...
    def _classify_momentum_strength(
        self, rsi: float, roc_5: float, roc_10: float
    ) -> str:
