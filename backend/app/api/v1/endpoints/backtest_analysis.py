...
class DayAnalysisData(BaseModel):
    ...
    comprehensive_technical_analysis: Optional[ComprehensiveTechnicalAnalysis] = None  # Added field
...
async def get_available_dates_by_run_id(
    ...
    try:
        ...
        if not actual_session_id:
            ...
            cursor.execute("SELECT DISTINCT session_id FROM daily_analysis_logs ORDER BY session_id DESC LIMIT 1")
            latest_session = cursor.fetchone()
            if latest_session:
                actual_session_id = latest_session[0]
            else:
                conn.close()
                raise HTTPException(status_code=404, detail=f"No sessions found in database")
...
async def get_day_analysis(
    ...
    symbol: Optional[str] = Query(None, description="Filter by symbol"),
    db_path: str = Query("backend/data/backtest_logs.db", description="Database path"),
    include_retrospective: bool = Query(True, description="Include LLM retrospective analysis")
) -> DayAnalysisResponse:
    """
    ...
    try:
        ...
        if log.get('comprehensive_technical_analysis'):
            comprehensive_technical_analysis = ComprehensiveTechnicalAnalysis(**log['comprehensive_technical_analysis'])
        ...
        if include_retrospective and llm_decision:
            retrospective_analysis = await generate_retrospective_analysis(historical_data)
...
async def generate_retrospective_analysis(day_data: DayAnalysisData) -> RetrospectiveAnalysis:
    """
    ...
    try:
        ...
        async def _generate_analysis():
            ...
            daily_return_text = f"{day_data.daily_return:+.2%}" if day_data.daily_return else "N/A"
            ...
            if day_data.technical_events:
                events_text = "\n".join([
                    ...
                    for event in day_data.technical_events[:3]  # Limit to 3 events to reduce prompt size
                ])
                print(f"📈 Number of technical events: {len(day_data.technical_events)}")
            else:
            ...
            confidence_text = f"{day_data.llm_decision.confidence:.1%}" if (day_data.llm_decision and day_data.llm_decision.confidence) else "No confidence data"
            reasoning_text = day_data.llm_decision.reasoning if (day_data.llm_decision and day_data.llm_decision.reasoning) else "No reasoning record"
            decision_text = day_data.llm_decision.decision_type if day_data.llm_decision else "No decision record"
            risk_level = day_data.llm_decision.risk_level if (day_data.llm_decision and day_data.llm_decision.risk_level) else "Not assessed"
...
"""
            ...
            try:
                ...
                llm_client = get_llm_client(temperature=0.8, max_tokens=1000)  # Reduced token limit
                
                # Get LLM response with timeout handling
                response = llm_client.invoke(context)
                commentary = response.content if hasattr(response, 'content') else str(response)
        ...
        try:
            commentary = await asyncio.wait_for(_generate_analysis(), timeout=timeout_seconds)
...
async def get_session_dates(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving session dates: {str(e)}")
